...
bcr <- read.csv('data/basic/fish_bcr.csv')
...
bcr <- merge(bcr, gra[,c('FishID', 'fish_id')], all=F, sort=F)
bbs <- merge(bcr, bbs, all=F, sort=F)
bbs <- bbs[which(bbs$bcr == 17 & bbs$year %in% c(2010:2021)),]
ebird <- merge(bcr, ebird, all=F, sort=F)
ebird <- ebird[which(ebird$bcr == 17 & ebird$year %in% c(2010:2021)),]
...
new_bbs <- bbs_t$new
bbs_cnt_ori <- as.matrix(bbs_t[,-c(1:8)])

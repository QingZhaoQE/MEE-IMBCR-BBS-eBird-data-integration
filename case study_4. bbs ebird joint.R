...
### Prepare data
gra <- gra[which(gra$fish_id %in% sort(unique(c(bbs$fish_id, ebird$fish_id)))),]
tem <- tem[which(tem$fish_id %in% sort(unique(c(bbs$fish_id, ebird$fish_id)))),]
...
year0 <- min(c(years_bbs, years_ebird))
...
for (i in 1:nsite) {
  for (t in 1:nyear) {
    if (length(c(bbs_cnt[which(sites_bbs == i & years_bbs == t),], ebird_cnt[which(sites_ebird == i & years_ebird == t)])) > 0) {
      Ni[i,t] <- max(c(bbs_cnt[which(sites_bbs == i & years_bbs == t),], ebird_cnt[which(sites_ebird == i & years_ebird == t)]))
    }
  } # t
} # i

...
nsite       <- 200 # total number of sites
...
### BBS
bbs_chi_mean <- 1.5  # mean scaling factor for BBS observation
...
#===============
### Sites
sites_imbcr <- rep(c(0,1), times=nsite/2)
sites_bbs <- rep(0, nsite)
sites_bbs[which(sites_imbcr == 0)] <- 1
sites_bbs[which(sites_imbcr == 1)] <- rep(c(0,1), times=length(which(sites_imbcr == 1))/2)

...
nsite       <- 225 # total number of sites
...
#===============
### Sites
sites_imbcr <- c(rep(c(0,1), times=100), rep(0, 25))
sites_bbs <- rep(0, nsite)
sites_bbs[which(sites_imbcr == 0)] <- 1
sites_bbs[which(sites_imbcr == 1)] <- c(rep(1,25), rep(0,75))
sites <- data.frame(sites_imbcr, sites_bbs)
#table(sites)
...
x[which(sites_imbcr==1),,1] <- rtruncnorm(n=nsite_imbcr*nyear, a=0, b=Inf, mean=0, sd=1)
...
data <- list(
  imbcr_cnt_sum=imbcr_cnt_sum, imbcr_cnt=imbcr_cnt, bbs_cnt=bbs_cnt
)
...
mcmcConf$removeSamplers(c('beta_lambda0', 'beta_rho', 'alpha_sigma', 'alpha_theta', 'alpha_bbs_chi'))
mcmcConf$printSamplers()
